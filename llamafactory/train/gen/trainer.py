            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        ...
                        # if (step % probe_every == 0) and (collected_batches < max_probe_batches):

                        # collected_batches += 1
                        model.eval()
                        
                        # (B1) SAM probe curves 수집
                        # collect_probe_curve_sam(model, inputs, labels, rho_list, ...) -> (curves[B,K], gnorm_probe)
                        rho_list = [0.0,  0.03, 0.05, 0.1]
                        curves, _ = collect_probe_curve_sam(
                            model, inputs, labels, rho_list, exclude_bias_norm=True
                        )  # curves: [B, K], rho_list[0]이 0이면 첫 열이 loss0
                        ...
                        with torch.no_grad():
                            ...
                            last_h = out_h.hidden_states[-1]             # [B,T,H]
                            emb = _masked_mean_last_hidden(last_h, inputs["attention_mask"])  # [B,H]
                                    # self.all_emb.append(emb.cpu())

                                # (B3) 결과 누적
                                # idx_list = inputs["__index__"].detach().cpu().tolist() if "__index__" in inputs else list(range(curves.size(0)))
                                # self.all_curves.append(curves.cpu())

                        # self.args.optim_name = self.optim_name
                        # self.args.hessian =  H_vals.max().item()
                        self.args.losses = curves.cpu().tolist() # per_sample_loss.detach().cpu().tolist()
                        # losses, cats는 step마다 이미 있는 값
                        self.args.rho_eff =  emb.cpu().tolist()
...
@torch.no_grad()
def collect_probe_curve_sam(model, inputs, labels, rho_list,
                            exclude_bias_norm=True) -> tuple[torch.Tensor, float]:
    """
    returns: (curves [B, len(rho_list)], grad_norm)
    - 현재 파라미터에서 이미 grad가 계산되어 있어야 함(방금 backward 후 등).
    - 가중치 백업→rho마다 perturb→forward(loss)→복구(다음 rho 전)→마지막에 원위치.
    """
    backup = {}
    backup_params_(model, backup)
    curves = []
    # grad norm (perturb 함수가 계산해서 반환)
    gnorm = perturb_along_grad_(model, rho=0.0, exclude_bias_norm=exclude_bias_norm)  # 0.0로 호출해도 gnorm 계산용
    restore_params_(model, backup)

    for rho in rho_list:
        backup_params_(model, backup)
        perturb_along_grad_(model, rho=rho, exclude_bias_norm=exclude_bias_norm)
        out = model(**{k: v for k, v in inputs.items() if k != "__index__"})
        loss_rho = per_sample_loss_from_logits(out.logits, labels)  # [B]
        curves.append(loss_rho.detach().cpu())
        restore_params_(model, backup)

    curves = torch.stack(curves, dim=1)  # [B, K]
    # 마지막에 확실히 복구
    restore_params_(model, backup)
    return curves, gnorm
...
@torch.no_grad()
def _masked_mean_last_hidden(hidden_states, attention_mask):
    """
    hidden_states: [B, T, H] (output_hidden_states=True일 때 마지막 레이어)
    attention_mask: [B, T]
    return: [B, H]
    """
    # mask [B,T,1]
    mask = attention_mask.unsqueeze(-1).to(hidden_states.dtype)
    num = (hidden_states * mask).sum(dim=1)                            # [B,H]
    den = mask.sum(dim=1).clamp_min(1e-6)                              # [B,1]
    return num / den



# ====== 유틸: SAM 스타일 perturb/복구 ======
def backup_params_(model, store: dict):
    store.clear()
    for p in model.parameters():
        if p.requires_grad:
            store[p] = p.data.clone()

@torch.no_grad()
def restore_params_(model, store: dict):
    for p in model.parameters():
        if p in store:
            p.data.copy_(store[p])

@torch.no_grad()
def perturb_along_grad_(model, rho: float, eps: float = 1e-12,
                        exclude_bias_norm: bool = True):
    # grad 방향 단위벡터 * rho 로 이동
    grads = []
    params = []
    for n, p in model.named_parameters():
        if (not p.requires_grad) or (p.grad is None):
    ...
    if len(grads) == 0:
        return 0.0
    # global grad norm
    gnorm = torch.norm(torch.stack([g.norm(p=2) for g in grads]), p=2)
    scale = rho / (gnorm + eps)
    for p, g in zip(params, grads):
        p.add_(g * scale)  # w <- w + rho * g / ||g||
    return float(gnorm.item())

# ====== 유틸: per-sample loss ======
def per_sample_loss_from_logits(logits, labels, ignore_index=-100):
    V = logits.size(-1)
    ...
    return (loss_tok * mask).sum(dim=1) / (mask.sum(dim=1) + 1e-8)  # [B]
